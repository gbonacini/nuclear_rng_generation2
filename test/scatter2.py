...
with open(''.join(sys.argv[1]), "rb") as inputFile:
       for binValues in iter(lambda: inputFile.read(2), ''):
                if len(binValues) != 2:
                    break

                xs.append(int.from_bytes(binValues[:1], byteorder='little'))
                ys.append(int.from_bytes(binValues[1:], byteorder='little'))
